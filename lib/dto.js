...
function inheritFn(SuperClass, mapping) {
  ...
  Class.__PLUGINS__ = SuperClass.__PLUGINS__ || {};
  Class.plugFn = fieldFn.bind(null, Class);

  return Class;
}

function plugFn(Class, plugin, options) {
  options = options || {};
  plugin = plugin.init(Class, options);

  if(Class.constructor.__PLUGINS__[plugin.name]) {
    throw new errors.PluginError('Plugin with name "' + plugin.name + '" already registered.');
  }

  Object.keys(plugin.methods || {}).forEach(function(methodName) {
    if(Class[methodName]) {
      throw new errors.PluginError('Plugin "' + plugin.name +
                                  '" cannot overwrite existing method "' + methodName + '"');
    }

    Class[methodName] = plugin.methods[name];
  });

  Class.__PLUGINS__[plugin.name] = plugin;
};
 ...
 * TODO: docs
 */
BaseDTO.plugFn = plugFn.bind(null, BaseDTO);
